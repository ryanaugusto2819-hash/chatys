...
import {
  ...
  AlertTriangle, Check, ChevronsUpDown, ChevronDown, ChevronUp, Clock3, History, Link2, Loader2, Megaphone, MessageCircle, MessageSquareText, PackageCheck, Play, Plus, Save, Search, ShieldCheck, ShoppingBag, ThumbsDown, ThumbsUp, Trash2,
} from 'lucide-react';
...
type TrainingQueueItem = {
  ...
  decision_feedback?: 'correct' | 'incorrect' | null;
  ...
  ai_trained_message_rules?: {
    id: string; example_message: string; context_notes: string; action_observation: string;
    expected_action: string; action_type: string; country_code: string;
  } | null;
};
type TrainedRule = {
                ...
                <div className="mt-6 space-y-6 border-t border-border pt-5">
                  ...
                  {trainingView === 'waiting' && <div className="space-y-3">
                    ...
                    {groupedTrainingQueue.length === 0 ? <div className="rounded-md border border-dashed border-border p-5 text-center text-sm text-muted-foreground">Nenhuma mensagem nova aguardando treinamento neste país.</div> : groupedTrainingQueue.map((group) => { const item = group[0]; return <div key={item.id} className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div className="flex flex-wrap items-center gap-2"><Badge>{nicheName(item.niche_id)}</Badge>{group.length > 1 && <><Badge variant="secondary">{group.length} mensagens semelhantes</Badge><span className="text-xs text-muted-foreground">Ao ensinar, este cenário será aplicado a todo o grupo.</span></>}</div>
                  ...
                  {trainingView === 'responses' && <div className="space-y-3">
                    ...
                    {visibleTrainedRules.length === 0 ? <div className="rounded-md border border-dashed border-border p-5 text-center text-sm text-muted-foreground">Nenhuma resposta aprendida neste país.</div> : visibleTrainedRules.map((rule) => <div key={rule.id} className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      {ruleConflicts[rule.id] && <div className="mb-2 flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>Possível conflito com {ruleConflicts[rule.id].length} regra{ruleConflicts[rule.id].length > 1 ? 's' : ''}.</strong> Os exemplos, país e condições de etiqueta são semelhantes; revise as instruções para deixar clara a diferença.</span></div>}
                      <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Badge>{nicheName(rule.niche_id)}</Badge><Switch checked={rule.active} onCheckedChange={(active) => setTrainedRules((current) => current.map((item) => item.id === rule.id ? { ...item, active } : item))} /><span className="text-sm font-medium text-foreground">{rule.active ? 'Ativa' : 'Desativada'}</span></div><Button type="button" variant="ghost" size="icon" title="Excluir regra" onClick={() => void deleteTrainedRule(rule.id)}><Trash2 className="h-4 w-4" /></Button></div>
                      ...
                      <div><label className="mb-1.5 block text-xs font-medium text-foreground">País</label><Select value={rule.country_code} onValueChange={(value: CountryFilter) => setTrainedRules((current) => current.map((item) => item.id === rule.id ? { ...item, country_code: value } : item))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{TRAINING_COUNTRIES.map((country) => <SelectItem key={country.code} value={country.code}>{country.label}{country.ddi ? ` (${country.ddi})` : ''}</SelectItem>)}</SelectContent></Select></div>
                       ...
                       <div><label className="mb-1.5 block text-xs font-medium text-foreground">Observação da ação</label><Textarea value={rule.action_observation} onChange={(event) => setTrainedRules((current) => current.map((item) => item.id === rule.id ? { ...item, action_observation: event.target.value } : item))} rows={3} placeholder="Motivo interno para a IA entender quando esta ação é adequada." /><p className="mt-1 text-xs text-muted-foreground">Uso interno: nunca será enviada ao cliente.</p></div>
                        ...
                        {['flow', 'reply_then_flow'].includes(rule.action_type) && <div><label className="mb-1.5 block text-xs font-medium text-foreground">Fluxo selecionado</label><FlowSearchSelect flows={flowsForNiche(rule.niche_id)} value={rule.flow_id} onChange={(value) => setTrainedRules((current) => current.map((item) => item.id === rule.id ? { ...item, flow_id: value } : item))} /></div>}
                  ...
                  {trainingView === "history" && <div className="space-y-3">
                    ...
                    {filteredHistory.length === 0 ? <div className="rounded-md border border-dashed border-border p-5 text-center text-sm text-muted-foreground">Nenhuma decisão encontrada.</div> : filteredHistory.map((item) => { const result = historyStatus(item); const matchedRule = snapshotRule(item) || item.ai_trained_message_rules || trainedRules.find((rule) => rule.id === item.matched_rule_id); return <div key={`history-${item.id}`} className="space-y-4 rounded-md border border-border bg-muted/20 p-4">
                      <div className="flex flex-wrap items-start justify-between gap-2"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium text-foreground">{item.conversations?.contact_name || 'Cliente sem nome'}</p><Badge>{nicheName(item.niche_id)}</Badge><Badge variant="secondary">{countryLabel(itemCountry(item))}</Badge>{snapshotTagNames(item).length ? snapshotTagNames(item).map((tag) => <Badge key={`history-${item.id}-${tag}`} variant="outline">{tag}</Badge>) : <Badge variant="outline">Sem tag</Badge>}</div><p className="text-xs text-muted-foreground">{item.conversations?.contact_phone || 'Telefone não informado'} · {new Date(item.processed_at || item.created_at).toLocaleString('pt-BR')} · {item.message_type}</p></div><div className="flex flex-wrap items-center gap-2"><Button type="button" variant="outline" size="sm" onClick={() => navigate(`/conversations/${item.conversation_id}`)}><MessageCircle className="mr-2 h-4 w-4" />Abrir conversa</Button>{Number(item.confidence) > 0 && <Badge variant="outline">{Math.round(Number(item.confidence) * 100)}% confiança</Badge>}<Badge variant={result.variant}>{result.label}</Badge></div></div>
                      ...
                      <div className="space-y-3 rounded-md border border-border bg-background p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold text-foreground">Base usada para decidir</p>{matchedRule && <Badge variant="outline">{countryLabel(matchedRule.country_code)}</Badge>}</div>{matchedRule ? <><div><p className="text-xs font-medium text-muted-foreground">Mensagem de exemplo da base</p><p className="mt-1 text-sm text-foreground whitespace-pre-wrap">{matchedRule.example_message}</p></div><div><p className="text-xs font-medium text-muted-foreground">Instrução e contexto cadastrados</p><p className="mt-1 text-sm text-foreground whitespace-pre-wrap">{matchedRule.context_notes || 'Nenhuma instrução adicional cadastrada.'}</p></div><div><p className="text-xs font-medium text-muted-foreground">Observação sobre a ação</p><p className="mt-1 text-sm text-foreground whitespace-pre-wrap">{matchedRule.action_observation || 'Nenhuma observação cadastrada.'}</p></div></> : <p className="text-sm text-muted-foreground">Nenhuma base foi selecionada para esta decisão.</p>}</div>
                    </div>; })}
                  </div>}
                </div>
              )}
