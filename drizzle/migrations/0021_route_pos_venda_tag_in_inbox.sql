    ...
    WHERE c.contact_phone NOT LIKE '%-group'
      ...
      AND (
        (
          ...
          AND NOT EXISTS (
            SELECT 1
            FROM contact_tags pos_ct
            JOIN tags pos_t ON pos_t.id = pos_ct.tag_id
            WHERE pos_ct.contact_phone = c.contact_phone
              AND pos_ct.workspace_id = c.workspace_id
              AND pos_t.workspace_id = c.workspace_id
              AND regexp_replace(btrim(pos_t.name), '\s+', ' ', 'g') ILIKE 'POS VENDA'
          )
        )
        OR (
          p_sector = 'pos_venda'
        ...
        OR (
          p_sector NOT IN ('', 'pos_venda')
          AND c.sector = p_sector
          AND NOT EXISTS (
            SELECT 1
            FROM contact_tags pos_ct
            JOIN tags pos_t ON pos_t.id = pos_ct.tag_id
      ...
      AND (
        p_connection_ids IS NOT NULL
        OR c.connection_config_id IS NULL
        OR NOT EXISTS (
          SELECT 1 FROM warmup_profiles wp
          WHERE wp.connection_config_id = c.connection_config_id
        )
      )
      AND (p_tag_id IS NULL OR EXISTS (
        SELECT 1
        FROM contact_tags ct
        WHERE ct.contact_phone = c.contact_phone
