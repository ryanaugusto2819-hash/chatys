...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
      FROM jsonb_to_recordset(p_edges) AS edge(source_node_id uuid, target_node_id uuid)
  ...
  UPDATE public.automation_flows
     SET name = btrim(p_name),
         ...
         manual_only = COALESCE(p_manual_only, false),
         niche_id = p_niche_id
   WHERE id = p_flow_id;

  DELETE FROM public.automation_edges WHERE flow_id = p_flow_id;
  DELETE FROM public.automation_nodes WHERE flow_id = p_flow_id;
  ...
  INSERT INTO public.automation_edges (
    id, flow_id, source_node_id, target_node_id, source_handle
  )
  SELECT edge.id,
         p_flow_id,
         edge.source_node_id,
         edge.target_node_id,
         NULLIF(btrim(edge.source_handle), '')
