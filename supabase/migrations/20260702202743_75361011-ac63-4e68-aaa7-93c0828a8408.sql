...
ALTER TABLE public.connection_configs ADD COLUMN IF NOT EXISTS sector text;
ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS sector text;
CREATE INDEX IF NOT EXISTS idx_conversations_sector ON public.conversations(sector);

CREATE OR REPLACE FUNCTION public.inherit_sector_from_connection()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NEW.sector IS NULL AND NEW.connection_config_id IS NOT NULL THEN
    SELECT cc.sector INTO NEW.sector
    FROM public.connection_configs cc WHERE cc.id = NEW.connection_config_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS inherit_sector_before_insert ON public.conversations;
CREATE TRIGGER inherit_sector_before_insert
BEFORE INSERT ON public.conversations
FOR EACH ROW EXECUTE FUNCTION public.inherit_sector_from_connection();

-- Drop older overload and recreate with p_sector param
DROP FUNCTION IF EXISTS public.get_inbox_page(integer, integer, text, text, uuid, uuid[], uuid, boolean, boolean);
DROP FUNCTION IF EXISTS public.get_inbox_page(integer, integer, text, text, uuid, uuid[], uuid, boolean, boolean, uuid);
...
CREATE OR REPLACE FUNCTION public.get_inbox_page(
  ...
  p_sector text DEFAULT ''::text
)
RETURNS TABLE(
  id uuid, contact_name text, contact_phone text, status text, tags text[],
  updated_at timestamp with time zone, assigned_agent_id uuid, last_message text,
  last_message_sender text, unread_count bigint, niche_id uuid,
  connection_config_id uuid, contact_tags jsonb, sector text, total_count bigint
)
