...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: recentMessages }, { data: tags }, { data: rules }] = await Promise.all([
      ...
      service.from("ai_trained_message_rules").select("id, example_message, context_notes, expected_action, action_type, official_response, response_messages, flow_id, required_tag_ids, excluded_tag_ids, requires_no_tags, country_code").eq("agent_config_id", config.id).eq("active", true).order("updated_at", { ascending: false }).limit(100),
    ]);
